...
public class MD2 {
    ...
    public MD2(String initialMessage) {
        ...
        rows = rowsMatrix();
        checksumMatrix = new int[rows][16];
        msgCheck = new String[rows+1][16];
        msgFinal = new String[(rows+1)*16];
        fasi = "";
    }

    public String encrypt() {
        String[][] matrix = new String[rows][16];
...
//        System.out.println("Matrice a interi ASCII");
    ...
    private String calculateHash(int[] digest) {
        ...
        for (int i = 0; i < 16; i++) {
            val += String.format("%02X",digest[i]);
        }
        val = val.toLowerCase();
    ...
    private void calculateFinalMessage(int[] checksum) {
        ...
        for (int i = 0; i < checksum.length; i++) {
            msgCheck[rows][i] = String.valueOf(checksum[i]);
        }
    }
    ...
    private void calculateChecksum(String[][] matrix) {
        ...
        for (int i = 0; i < 16; i++) {
            checksum[i] = checksumMatrix[rows-1][i];
        }
    }
...
//        System.out.println("");
    ...
    private String[][] copyMatrix(String[][] matrix){
        String[][] copyMatrix = new String[rows][16];
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
    ...
    /**
     * Calcola le righe da inserire nella matrice
     * @return numero righe
     */
    private int rowsMatrix(){
        if(initialMessage.length()%16 == 0) return initialMessage.length()/16;
        return (int) (Math.floor(initialMessage.length()/16) + 1);
    }
}
