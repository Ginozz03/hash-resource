...
import service.sha1.operazionearray.OperazioniArray;
import service.sha1.operazionebit.OperazioniBit;
...
public class SHA1 {
    ...
    /**
     * aggiunge zero alla fine della stringa di bit per fare in modo che la
     * lunghezza della stringa divisa per 512 dia come risultato 448
     * 
    ...
    /**
     * genera una stringa a partire dalla lunghezza del messaggio binario, la
     * lunghezza viene poi convertita in binario e vengono aggiunti zero a sinistra
     * per renderla di lunghezza 64 bit
     * 
     * @param messaggioBinario l'array di stringhe binarie
     * @return la stringa binaria di 64 bit che rappresenta la lunghezza del
     *         messaggio
     */
    ...
    /**
     * cambia i valori delle costanti h definite all'inizio della classe scorrendo
     * la matrice parole effettuando operazioni sui bit
     * 
    ...
    private static void scorriParole(String[][] parole) {
        ...
        for (int i = 0; i < parole.length; i++) {
            for (int j = 0; j < 80; j++) {
                if (j < 20) {
                    String bAndC = OperazioniBit.and(b, c);
                    String notB = OperazioniBit.and(OperazioniBit.not(b), d);
                    f = OperazioniBit.or(bAndC, notB);
                    k = "01011010100000100111100110011001";
                } else if (j < 40) {
                    String bXorC = OperazioniBit.xor(b, c);
                    f = OperazioniBit.xor(bXorC, d);
                    k = "01101110110110011110101110100001";
                } else if (j < 60) {
                    String bAndC = OperazioniBit.and(b, c);
                    String bAndD = OperazioniBit.and(b, d);
                    String cAndD = OperazioniBit.and(c, d);
                    String bAndCOrBAndD = OperazioniBit.or(bAndC, bAndD);
                ...
                String parola = parole[i][j];
                String tempA = OperazioniBit.sommaBinaria(OperazioniBit.ruotaASinistra(a, 5), f);
                String tempB = OperazioniBit.sommaBinaria(tempA, e);
                String tempC = OperazioniBit.sommaBinaria(tempB, k);
                String temp = OperazioniBit.sommaBinaria(tempC, parola);
    ...
    /**
     * converte i valori delle costanti h in esadecimale e li concatena uno dopo
     * l'altro
     * 
